...
class Model:
    ...
    def cultural_question(self):
        ...
        self.messages.append({"role": "user", "parts": ["You must return a full JSON object that strictly follows the schema. Include *all* of the schemas' attributes"]})
        response = self.model.generate_content(
            contents = self.messages,
            generation_config={
                "response_mime_type": "application/json",
                "response_schema": CultureProfile,
                "temperature": 0,
            }
        )
    ...
    def RAG_Response(self,user_input):
        ...
        response = Chunking.LLM_Response(user_input,FilePath)
